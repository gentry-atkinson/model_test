# -*- coding: utf-8 -*-
"""TWristAR Dataset Demo.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1oD2Hwz0wmyWZqj07RHslUoRmjg_N7jXo

#TWristAR Dataset Demo.ipynb
Loads the 3 subject e4 wristband HAR dataset from Zenodo repository, converts the data into numpy arrays of X (data), y(labels), sub (subject numbers), then performs a hold-one-subject-out evaluation of a 1D-CNN.
...
>sub = {1,2,3,...} # subject number 

y and sub are column vectors with number of rows equal to samples. This is an intermediate representation which used to build the train/validate/test arrays.
...
Additional and updated code may be found at our [IMICS lab git repository](https://github.com/imics-lab/).
...
You may need to update with the published path of the dataset in zenodo.
"""
...
if (TWristAR_version == '1.0' and not path.exists('./TWristAR.zip')):
    system("wget 'https://zenodo.org/record/5911808/files/TWristAR.zip'") # type: ignore
elif (TWristAR_version == 'local'):  # use local version for debugging
    system("cp -r /content/drive/MyDrive/Datasets/TWristAR.zip /content") # type: ignore
else:
...
"""#e4_load_dataset 
Separates the train and test groups by subject, drops component acceleration, and one-hot-encodes the activity labels.
Usually this calls get_X_y_sub() but here we are calling it once and using the saved version to speed up execution.
"""
...
def e4_load_dataset(
    ...
    """for this version I am using global variables for X, y, sub - see below.
    Usually calls e4_get_X_y_sub and processes the returned arrays by separating
    into _train, _validate, and _test arrays for X and y based on split_subj
    dictionary."""

    # 
    X, y, sub, xys_info = get_X_y_sub(zip_flist = e4_flist)
    X = np.load('my_X.npy') # this creates a local X and y from saved version
    y = np.load('my_y.npy') # can't use global, X & y are modified so loop fails
    #global sub # need to declare global to give function access
...
# this model has not been extensively tuned
...
def train_1D_CNN(model,
    ...
    MAX_EPOCHS = 100): # Max number run unless earlystopping callback fires
...
"""#Run the experiment
Process the dataset, separate into train/test groups using hold-one-subject out.  The model is trained on two of the subject's data and tested on the remaining subject.
"""

if __name__ == '__main__':
    print("Downloading and processing TWristAR e4 wristband HAR dataset")

    # Normally load_dataset calls get_X_y_sub but by storing the ndarrays globally
    # the train/test split can be changed without reprocessing the dataset.
    # The dataset will need to be reprocessed if the sliding window is changed.
    e4_flist = ['sub1/1574621345_A01F11.zip',
                'sub1/1574622389_A01F11.zip',
                'sub1/1574624998_A01F11.zip',
                'sub2/1633107019_A01F11.zip',
                ...
                'sub3/1633711821_A01F11.zip']
    X, y, sub, xys_info = get_X_y_sub(zip_flist = e4_flist)
    # This is a bit of hack since I'm just doing this to save time and setting
    # X, y to globals allows them to be modified by e4_load_dataset
    np.save('my_X', X)
    np.save('my_y', y)
    # Hold each of the subjects out - just doing it manually here, for larger
    # number of subjects please see subject_split_generator.ipynb in
    # https://github.com/imics-lab/load_data_time_series
    results = 'Hold-one-subject out results\n'
    results += 'Train   Test   Acc    TTime\n'
    my_split_subj = (dict(train_subj = [1,2], validation_subj = [],test_subj = [3]),
                    dict(train_subj = [2,3], validation_subj = [],test_subj = [1]),
                    dict(train_subj = [1,3], validation_subj = [],test_subj = [2]))
    for sub_dict in my_split_subj: # Repeat for each entry
        print("Current Pass: Training with",sub_dict['train_subj'],"Testing on",sub_dict['test_subj'])
        results += str(sub_dict['train_subj']) +"  " + str(sub_dict['test_subj'])
        x_train, y_train, x_test, y_test = e4_load_dataset(split_subj = sub_dict)

        model = build_1D_CNN()
        start_time = time.time()
        history = train_1D_CNN(model)
        end_time = time.time()
        train_time = str(timedelta(seconds = (end_time - start_time))).split('.', 2)[0]

        #print("Final Accuracy: %0.3f" % history.history['val_accuracy'][-1])
        results += "   %0.3f" % history.history['val_accuracy'][-1]
        results += "  " + train_time + "\n"
